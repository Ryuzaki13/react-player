...
type ReactPlayer = React.ForwardRefExoticComponent<Omit<ReactPlayerProps, "ref"> & React.RefAttributes<HTMLVideoElement>> &
	Partial<{
	...
	const ReactPlayer: ReactPlayer = React.forwardRef((_props, ref) => {
		...
		return (
			<Wrapper slot={slot} className={className} style={{ width, height, ...style }}>
				<UniversalSuspense fallback={fallback}>{showPreview ? renderPreview(src) : renderActivePlayer(src)}</UniversalSuspense>
			</Wrapper>
		);
	});
