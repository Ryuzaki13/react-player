...
import { canPlay as defaultCanPlay } from "./patterns";
import type { ReactPlayerProps } from "./types";

const ForwardChildren = ({ children }: { children?: ReactNode }) => children;

export const ReactPlayer: ForwardRefExoticComponent<Omit<ReactPlayerProps, "ref"> & RefAttributes<HTMLVideoElement>> = forwardRef(
	(props, ref) => {
		const { src, slot, className, style, width, height, fallback, wrapper, canPlay = defaultCanPlay } = { ...defaultProps, ...props };
			...
			<Wrapper slot={slot} className={className} style={{ width, height, ...style }}>
				<UniversalSuspense fallback={fallback}>
					<Player
						...
						style={
							wrapper ? { display: "block", width: "100%", height: "100%" } : { display: "block", width, height, ...style }
						}
					/>
				</UniversalSuspense>
			</Wrapper>
